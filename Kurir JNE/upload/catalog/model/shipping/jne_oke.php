...
class ModelShippingJNEOke extends Model {    
  	public function getQuote($address) {
		...
		foreach ($query->rows as $result) {
			if ($this->config->get('jne_oke_' . $result['geo_zone_id'] . '_status')) {
			...
			if ($status) {
				...
				//$rates = explode(',', $this->config->get('jne_oke_' . $result['geo_zone_id'] . '_rate'));
				$rates = preg_split('/\r\n/', $this->config->get('jne_oke_' . $result['geo_zone_id'] . '_rate'));
				...
				if ((string)$cost != '') { 
					$quote_data['jne_oke_' . $result['geo_zone_id']] = array(
						'code'         => 'jne_oke.jne_oke_' . $result['geo_zone_id'],
						...
						'cost'         => $cost,
						'tax_class_id' => $this->config->get('jne_oke_tax_class_id'),
						'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('jne_oke_tax_class_id'), $this->config->get('config_tax')))
					);	
				}
			}
		}
		...
		if ($quote_data) {
      		$method_data = array(
        		...
        		'quote'      => $quote_data,
				'sort_order' => $this->config->get('jne_oke_sort_order'),
        		'error'      => false
      		);
		}
